...
func initialize_save_request(save_request: SaveRequest) -> void:
	var attribute_prefix := _get_attribute_prefix(save_request.character, save_request.attribute)
	_entry.type_text("%s: Choose Hit Dice…" % attribute_prefix)

func initialize_save_result(save_result: SaveResult) -> void:
	var attribute_prefix := _get_attribute_prefix(save_result.character, save_result.attribute)
	var message := "No Hit Dice!"
	if not save_result.highest_dice.is_empty():
		var dice_results := _get_dice_results(save_result)
		var difficulty := _get_difficulty(save_result)
...
func _get_attribute_prefix(character: Character, attribute: CharacterAttribute) -> String:
	var hint := "%s: %d" % [attribute, character.get_attribute_score(attribute)]
...
func _get_dice_results(save_result: SaveResult) -> String:
	...
	for index: int in highest_dice.size():
		...
		var color := die.get_die_color(save_result.difficulty)
...
func _get_difficulty(save_result: SaveResult) -> String:
	var difficulty := "%d" % save_result.result
	...
	if save_result.difficulty != SaveResult.Outcome.NORMAL:
		var hint := "Difficulty: %d → %s" % [save_result.difficulty, SaveResult.Outcome.find_key(save_result.save_outcome)]

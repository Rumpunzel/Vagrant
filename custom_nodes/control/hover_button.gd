...
@export var hover_neighbors: Array[HoverButton] :
	set(new_hover_neighbors):
		hover_neighbors = new_hover_neighbors
		_hover_fade.hover_neighbors.assign(hover_neighbors.map(func(button: HoverButton) -> HoverFade: return button._hover_fade))
...
func _init() -> void:
	...
	_hover_fade.control_to_fade = self
